...
export const donateFood = async (req, res) => {
  try {
    ...
    const { foodType, weight, expiryDate, pickupLocation, foodImages, notes } =
      req.body;
    ...
    if (isNaN(weight)) {
      return res
        .status(400)
        .json({ message: "Weight must be a number", success: false });
    }
    ...
    if (isNaN(expiryDateObj.getTime())) {
      return res
        .status(400)
        .json({ message: "Invalid expiry date", success: false });
    }
    ...
    return res
      .status(201)
      .json({ message: "Food donated successfully", success: true });
  } catch (error) {
    console.log(error);
    return res
      .status(500)
